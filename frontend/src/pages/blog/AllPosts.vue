  ...
  <PageBase :label="`${blogTitle} posts`" icon="fc-opened-folder">
    <div class="min-w-full min-h-full gap-16 grid grid-cols-2 p-8">
      <template v-if="!articlesStore.isDownloading">
        <router-link v-for="post in posts" :key="post.id" :to="`/blog/${blogSite}/${post.id}`">
          <div
            class="bg-[--ui-bg-alt] rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer"
          >
            <figure>
              <img
            ...
            <div class="p-4">
              ...
              <p class="text-sm text-[--ui-text-muted]">
                {{ getFormatedDate(post.publishDate) }}
              </p>
            </div>
          </div>
        </router-link>
